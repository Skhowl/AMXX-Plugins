...
{
	register_plugin("DM: Spawn Team Preset", DM_VERSION_STR_LONG, "schmurgel1983");
...
{
	if (DM_IsFreeForAllEnabled() || !DM_RegisterSpawnMode("team_preset", "DM_Spawn_TeamPreset"))
	{
		state deactivated;
		return;
	}
	...
	cvar_fMin = register_cvar("dm_team_preset_min", "64.0");
	cvar_fMax = register_cvar("dm_team_preset_max", "256.0");
...
public DM_Spawn_TeamPreset(id, freezetime, roundend)
{
	...
	switch (g_iTeamID[id])
	{
		case 1: // get_players(players, num, "ae", "TERRORIST"); // dosen't work all time (1.8.2.dev.hg24)
		{
			#if AMXX_VERSION_NUM < 183
		...
		case 2: // get_players(players, num, "ae", "CT"); dosen't work all time (1.8.2.dev.hg24)
		{
			#if AMXX_VERSION_NUM < 183
			for (i = 1; i <= g_iMaxPlayers; i++)
			#else
	...
	if (num < 2 && g_iTotalSpawns > 1)
	{
		...
		for (i = n + 1; /*no condition*/; i++)
		{
			...
			if (g_iSpawnTeamID[i] > DM_TEAM_UNASSIGNED && g_iTeamID[id] != g_iSpawnTeamID[i])
			{
				if (i == n) break;
				else continue;
			}
	...
	{
		new teamid, oldmate[32], z, Float:origin[3];
		new Float:fmin = get_pcvar_float(cvar_fMin);
		new Float:fmax = get_pcvar_float(cvar_fMax);
		...
		for (x = 1; x < num; x++)
		{
			...
			oldmate[n] = teamid = players[n];
			
			pev(teamid, pev_origin, origin);
			
			z = random_num(0, g_iTotalSpawns - 1);
			
			for (i = z + 1; /*no condition*/; i++)
			{
				if (i >= g_iTotalSpawns) i = 0;
				...
				if (!g_iSpawnTeamID[i] && (fmin <= get_distance_f(origin, g_fOrigin[i]) <= fmax))
				{
					if (!is_hull_vacant(g_fOrigin[i], HULL_HUMAN))
					{
						if (i == z) break;
				...
				else if (g_iSpawnTeamID[i] == g_iTeamID[id] && !random(10) && is_hull_vacant(g_fOrigin[i], HULL_HUMAN))
				{
					x = num;
