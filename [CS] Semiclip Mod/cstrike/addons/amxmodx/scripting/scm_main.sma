	...
	switch (type)
	{
		...
		case 3: /* Random around own place */
		{
			...
			for (i = iSpawnPoint + 1; /*no condition*/; i++)
			{
				...
				UTIL_Vector_Add(flOrigin, RANDOM_OWN_PLACE[i], flOriginFinal)
...
public DisableTask(taskid)
{
	...
	{
		new id, bs_IsStuck, iCts, iTerrorists, iTeam, iEnemy
		for (id = 1; id <= g_iMaxPlayers; id++)
		{
...
PlayerWillStuck(id)
{
	...
	for (i = 1; i <= g_iMaxPlayers; i++)
	{
		if (!get_bitsum(bs_IsAlive, i) || get_bitsum(bs_IsDying, i) || i == id || !is_player_stuck(i))
			continue
...
public MessageTeamInfo(msg_id, msg_dest)
{
	...
	else if (PlayerWillStuck(id)) DoRandomSpawn(id, c_iUnstuck)
}
