...
#define scm_print_color client_print_color
#else
#define print_team_default  0
#define print_team_grey    -1
#define print_team_red     -2
#define print_team_blue    -3
...
public plugin_init()
{
	...
	/* Multi-Language */
	#if AMXX_VERSION_NUM >= 183
	register_dictionary("scm_entity_editor_183.txt");
	#else
	register_dictionary_colored("scm_entity_editor.txt");
	#endif
...
public clcmd_menu(id)
{
	if (get_user_flags(id) & ADMIN_ACCESS_LEVEL)
		...
		scm_print_color(id, print_team_default, "^4[SCM: Entity Editor]^1 %L.", id, "NO_ACC_COM");
}
...
public MainMenuHandler(id, key)
{
	switch (key)
	{
		...
		{
			if (LibraryExists("cs_semiclip_mod", LibType_Library) || LibraryExists("zp_team_semiclip", LibType_Library))
			{
				if (SaveIniFile(true))
				{
					if (scm_load_ini_file())
					{
						scm_print_color(id, print_team_blue, "^4[SCM: Entity Editor]^3 %L", id, "SCM_CHAT_SAVED", g_szMapName);
						#if defined SOUND_EFFECTS
					...
					{
						scm_print_color(id, print_team_red, "^4[SCM: Entity Editor]^3 %L", id, "SCM_CHAT_FAILED", g_szMapName);
						#if defined SOUND_EFFECTS
				...
				{
					scm_print_color(id, print_team_red, "^4[SCM: Entity Editor]^3 %L", id, "SCM_CHAT_FAIL_SAVE", g_szMapName);
					#if defined SOUND_EFFECTS
					client_cmd(id, "spk buttons/button11");
					#endif // SOUND_EFFECTS
				}
			...
			{
				scm_print_color(id, print_team_red, "^4[SCM: Entity Editor]^3 %L", id, "SCM_CHAT_FAIL_APPLY", g_szMapName);
				#if defined SOUND_EFFECTS
				client_cmd(id, "spk buttons/button11");
				#endif // SOUND_EFFECTS
			}
...
public ListMenuHandler(id, menu, item)
{
	...
	if (!g_iAbsBoxEnt[id])
	{
		...
		{
			scm_print_color(id, print_team_red, "^4[SCM: Entity Editor]^3 %L", id, "SCM_CHAT_FAIL_CREATE");
...
public AbsBoxThink(entity)
{
	...
	if (DATA_ENABLE_ENTITY)
	{
		...
		GetEntityCenterAbs(iEntityIndex, flCenter, flAbsMin, flAbsMax);
...
GetEntityCenterAbs(const iIndex, Float:flCenter[3], Float:flAbsMin[3], Float:flAbsMax[3])
{
	pev(iIndex, pev_absmin, flAbsMin);
	pev(iIndex, pev_absmax, flAbsMax);
...
scm_print_color(id, iColor=print_team_default, const szMsg[], any:...)
{
	...
	if (iColor < print_team_blue || iColor > print_team_default)
	{
	...
	if (id)
	{
		...
		if (iColor != print_team_default)
		{
			...
			SendTeamInfo(id, id, g_szTeamName[abs(iColor)]);
			
			// Send the message
			SendSayText(id, id, szMessage);
			
			// restore TeamInfo
			SendTeamInfo(id, id, szTeam);
		}
		else
		{
			SendSayText(id, id, szMessage);
	...
	else
	{
		...
		if (!iMlNumber)
		{
			...
			if (iColor != print_team_default)
			{
				...
				SendTeamInfo(0, iFool, g_szTeamName[abs(iColor)]);
				SendSayText(0, iFool, szMessage);
				SendTeamInfo(0, iFool, szTeam);
			}
			else
			{
				SendSayText(0, iFool, szMessage);
		...
		else
		{
			...
			{
				get_user_team(iFool, szTeam, charsmax(szTeam));
				copy(szFakeTeam, charsmax(szFakeTeam), g_szTeamName[abs(iColor)]);
			}
			...
			for (i = 0; i < iNum; i++)
			{
				...
				if (iColor != print_team_default)
				{
					SendTeamInfo(id, iFool, szFakeTeam);
					SendSayText(id, iFool, szMessage);
					SendTeamInfo(id, iFool, szTeam);
				}
				else
				{
					SendSayText(id, iFool, szMessage);
...
SendTeamInfo(iReceiver, iPlayerId, szTeam[])
{
	static iTeamInfo = 0;
	if (!iTeamInfo)
	{
...
SendSayText(iReceiver, iPlayerId, szMessage[])
{
	static iSayText = 0;
	if (!iSayText)
	{
...
register_dictionary_colored(const filename[])
{
	...
	while (!feof(fp))
	{
		...
		else if (szBuffer[0])
		{
			...
			{
				while(replace(szTranslation, charsmax(szTranslation), "x04", "^4")) { /* loop */ }
				while(replace(szTranslation, charsmax(szTranslation), "x03", "^3")) { /* loop */ }
				while(replace(szTranslation, charsmax(szTranslation), "x01", "^1")) { /* loop */ }
