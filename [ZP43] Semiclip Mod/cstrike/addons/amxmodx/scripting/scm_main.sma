...
#define MAX_REG_SPAWNS  32	/* Max cached regular spawns per team ¬ 24 */
...
/*================================================================================
 ...
 unstuck 1 need rework (line 1399) is_hull_vacant dosent work properly
 unstuck 3 need rework (line 1471) is_hull_vacant dosent work properly
 zp_round_started need rework (line 681)
...
new cvar_iSemiclipBlockTeam,
	...
	cvar_iSemiclipUnstuckRoundStart,
...
new Float:g_flSpawnsHuman[MAX_REG_SPAWNS][3],
	...
	Float:g_flSpawnsShared[MAX_REG_SPAWNS*2][3],
...
public plugin_init()
{
	...
	bind_pcvar_num(create_cvar("semiclip_unstuck_zp_round_start", "4", _, _, true, 0.0, true, 4.0), c_iUnstuckRoundStart)
	...
	cvar_iSemiclipUnstuckRoundStart = register_cvar("semiclip_unstuck_zp_round_start", "4")
...
public plugin_unpause()
{
	...
	for (new id = 1; id <= g_iMaxPlayers; id++)
	{
		...
		g_iTeam[id] = zp_get_user_zombie(id) ? SC_TEAM_ZOMBIE : SC_TEAM_HUMAN
		g_iSpectating[id] = id
...
public zp_round_started(gamemode, id)
{
	...
	{
		#if AMXX_VERSION_NUM >= 183
		case ZP_MODE_INFECTION: if (c_iDisableInfection) goto Label_Default;
		case ZP_MODE_NEMESIS: if (c_iDisableNemesis) goto Label_Default;
		case ZP_MODE_SURVIVOR: if (c_iDisableSurvivor) goto Label_Default;
		case ZP_MODE_SWARM: if (c_iDisableSwarm) goto Label_Default;
		case ZP_MODE_MULTI: if (c_iDisableMultiple) goto Label_Default;
		case ZP_MODE_PLAGUE: if (c_iDisablePlague) goto Label_Default;
		case NM_MODE_SNIPER: if (c_iDisableSniper) goto Label_Default;
		case NM_MODE_ASSASSIN: if (c_iDisableAssassin) goto Label_Default;
		case NM_MODE_ARMAGEDDON: if (c_iDisableArmageddon) goto Label_Default;
		case NM_MODE_VERSUS: if (c_iDisableVersus) goto Label_Default;
		case NM_MODE_APOCALYPSE: if (c_iDisableApocalypse) goto Label_Default;
		case NM_MODE_HUNT: if (c_iDisableHunt) goto Label_Default;
		case NM_MODE_NIGHTMARE: if (c_iDisableNightmare) goto Label_Default;
		#else
		case ZP_MODE_INFECTION: if (get_pcvar_num(cvar_iDisableInfection)) goto Label_Default;
		case ZP_MODE_NEMESIS: if (get_pcvar_num(cvar_iDisableNemesis)) goto Label_Default;
		case ZP_MODE_SURVIVOR: if (get_pcvar_num(cvar_iDisableSurvivor)) goto Label_Default;
		case ZP_MODE_SWARM: if (get_pcvar_num(cvar_iDisableSwarm)) goto Label_Default;
		case ZP_MODE_MULTI: if (get_pcvar_num(cvar_iDisableMultiple)) goto Label_Default;
		case ZP_MODE_PLAGUE: if (get_pcvar_num(cvar_iDisablePlague)) goto Label_Default;
		case NM_MODE_SNIPER: if (get_pcvar_num(cvar_iDisableSniper)) goto Label_Default;
		case NM_MODE_ASSASSIN: if (get_pcvar_num(cvar_iDisableAssassin)) goto Label_Default;
		case NM_MODE_ARMAGEDDON: if (get_pcvar_num(cvar_iDisableArmageddon)) goto Label_Default;
		case NM_MODE_VERSUS: if (get_pcvar_num(cvar_iDisableVersus)) goto Label_Default;
		case NM_MODE_APOCALYPSE: if (get_pcvar_num(cvar_iDisableApocalypse)) goto Label_Default;
		case NM_MODE_HUNT: if (get_pcvar_num(cvar_iDisableHunt)) goto Label_Default;
		case NM_MODE_NIGHTMARE: if (get_pcvar_num(cvar_iDisableNightmare)) goto Label_Default;
		#endif
		default:
		{
			Label_Default:
			DisableOnGamemode(id);
		}
	}
	
	new bs_IsStuck, iHumans, iZombies, iTeam, iEnemy
	for (id = 1; id <= g_iMaxPlayers; id++)
	{
		if (!get_bitsum(bs_IsAlive, id) || get_bitsum(bs_IsDying, id) || !is_player_stuck(id))
			continue
		
		add_bitsum(bs_IsStuck, id)
		log_amx("%d (%d) is stucking", id, g_iTeam[id])
		
		switch (g_iTeam[id])
		{
			case SC_TEAM_ZOMBIE: iZombies++
			case SC_TEAM_HUMAN: iHumans++
		}
	}
	
	if (iHumans == iZombies) iTeam = random_num(SC_TEAM_ZOMBIE, SC_TEAM_HUMAN)
	else if (iZombies > iHumans) iTeam = SC_TEAM_ZOMBIE
	else iTeam = SC_TEAM_HUMAN
	
	log_amx("zp_round_started z:%d, h:%d, t:%d", iZombies, iHumans, iTeam)
	for (id = 1; id <= g_iMaxPlayers; id++)
	{
		if (!get_bitsum(bs_IsStuck, id))
		{
			log_amx("%d skipped alive:%s dying:%s stuck:%s", id, get_bitsum(bs_IsAlive, id) ? "yes" : "no", get_bitsum(bs_IsDying, id) ? "yes" : "no", is_player_stuck(id) ? "yes" : "no")
			continue
		}
		...
		if (g_iTeam[id] == iTeam)
		{
			...
			{
				log_amx("%d lead unstuck", id)
				DoRandomSpawn(id, c_iUnstuckRoundStart)
			}
			log_amx("%d lead not unstuck", id)
			continue
		}
		...
		if (c_iButton)
		{
			...
			{
				log_amx("%d enemy unstuck no button pressed", id)
				DoRandomSpawn(id, c_iUnstuckRoundStart)
				continue
			}
			else if (iEnemy && !get_bitsum(bs_InButton, iEnemy))
			{
				log_amx("%d enemy unstuck no enemy button pressed", id)
				DoRandomSpawn(iEnemy, c_iUnstuckRoundStart)
			}
		...
		if (!c_iEnemies)
		{
			log_amx("%d unstuck no button cvar", id)
			DoRandomSpawn(id, c_iUnstuckRoundStart)
		}
	}
}
...
{
	/* Unstuck anybody */
	new iHull, iSpawnPoint, iNum, bool:bNoSpawnsLeft
	...
	for (id = 1; id <= g_iMaxPlayers; id++)
	{
		...
		switch (c_iUnstuckRoundStart)
		{
			case 1: /* Regular spawns */
			{
				if (!g_iSpawnCountShared || bNoSpawnsLeft)
					goto Label_Random
				
				for (iNum = iSpawnPoint + 1; /*no condition*/; iNum++)
				{
					if (iNum >= g_iSpawnCountShared)
						iNum = 0
					
					iSpawnPoint++
					if (is_hull_vacant(g_flSpawnsShared[iNum], iHull))
					{
						engfunc(EngFunc_SetOrigin, id, g_flSpawnsShared[iNum])
						break
					}
			...
			case 2: /* CSDM */
			{
				if (!g_iSpawnCountCSDM || bNoSpawnsLeft)
					goto Label_Random
				
				for (iNum = iSpawnPoint + 1; /*no condition*/; iNum++)
				{
					if (iNum >= g_iSpawnCountCSDM)
						iNum = 0
					
					iSpawnPoint++
					if (is_hull_vacant(g_flSpawnsCSDM[iNum], iHull))
					{
						engfunc(EngFunc_SetOrigin, id, g_flSpawnsCSDM[iNum])
						break
					}
			...
			case 3: /* Random around own place */
			{
				...
				for (iNum = iSpawnPoint + 1; /*no condition*/; iNum++)
				{
					...
					flOriginFinal[0] = flOrigin[0] + RANDOM_OWN_PLACE[iNum][0]
					flOriginFinal[1] = flOrigin[1] + RANDOM_OWN_PLACE[iNum][1]
					flOriginFinal[2] = flOrigin[2]
					
					engfunc(EngFunc_TraceLine, flOrigin, flOriginFinal, DONT_IGNORE_MONSTERS, id, 0)
					...
					do
					{
						if (is_hull_vacant(flOriginFinal, iHull))
						{
							...
							engfunc(EngFunc_SetSize, id, flMins, flMaxs)
							break
						}
			...
			case 4: /* Trespass */
			{
				new iPlayer, iPlayers[32]
				iNum = find_sphere_class(id, "player", 85.10, iPlayers, g_iMaxPlayers)
...
DisableOnGamemode(id)
{
	...
	for (id = 1; id <= g_iMaxPlayers; id++)
	{
		...
		if (is_player_stuck(id))
			DoRandomSpawn(id, (c_iUnstuckRoundStart == 4) ? 3 : c_iUnstuckRoundStart)
	}
}

public zp_user_humanized_post(id)
{
	g_iTeam[id] = SC_TEAM_HUMAN
	g_iSpectating[id] = id
	log_amx("%d humanized_post", id)
	
	if (g_bRoundStarted && TeamInfoUnstuck(id))
	{
...
{
	g_iTeam[id] = SC_TEAM_ZOMBIE
	g_iSpectating[id] = id
	log_amx("%d infected_post", id)
...
public CacheCvars(entity)
{
	...
	c_iUnstuckRoundStart = clamp(get_pcvar_num(cvar_iSemiclipUnstuckRoundStart), 0, 4)
...
public LoadSpawns()
{
	...
	while ((iEnt = find_ent_by_class(iEnt, HUMAN_SPAWN_ENTITY_NAME)) != 0)
	{
		...
		g_flSpawnsShared[g_iSpawnCountShared][0] = flOrigin[0]
		g_flSpawnsShared[g_iSpawnCountShared][1] = flOrigin[1]
		g_flSpawnsShared[g_iSpawnCountShared][2] = flOrigin[2]
...
DoRandomSpawn(id, type)
{
	...
	static iHull, iSpawnPoint, i, Float:fMins[3], Float:fMaxs[3]
	iHull = (pev(id, pev_flags) & FL_DUCKING) ? HULL_HEAD : HULL_HUMAN
				...
				case SC_TEAM_ZOMBIE: /* ZOMBIES */
				{
					...
					for (i = iSpawnPoint + 1; /*no condition*/; i++)
					{
						...
						if (is_hull_vacant(g_flSpawnsZombie[i], iHull))
						{
							...
							engfunc(EngFunc_SetSize, id, fMins, fMaxs)
							break
						}
