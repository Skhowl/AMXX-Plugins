...
new const item_name[] = { "sock_of_the_feather" };
new const item_chat[] = { "sock" };
...
{
	register_plugin("DM Item: Sock of the Feather", "1.0.0", "schmurgel1983");
...
public plugin_init() <enabled>
{
	...
	dm_item_sock_gravity = register_cvar("dm_item_sock_gravity", "0.5");
}
...
public DM_PlayerSpawn_Post(id, freezetime, roundend) <enabled>
{
	if (get_bitsum(bs_HaveItem, id))
		set_pev(id, pev_gravity, get_pcvar_float(dm_item_sock_gravity));
}
...
public Activate(id)
{
	if (!get_bitsum(bs_HaveItem, id))
	{
		...
		{
			#if AMXX_VERSION_NUM < 183
			dm_print_color(id, DontChange, "^4[%s]^1 Your gravity is reduced by^4 %d%%^1!", name, floatround(100 - get_pcvar_float(dm_item_sock_gravity) * 100));
			#else
			client_print_color(id, print_team_default, "^4[%s]^1 Your gravity is reduced by^4 %d%%^1!", name, floatround(100 - get_pcvar_float(dm_item_sock_gravity) * 100));
			#endif
		}
	}
